...
updatedAt: "2023-02-08T16:30:04.093Z"
---
The complete user guide for alerts and setting up alert rules in the Fiddler UI is provided [here](https://docs.fiddler.ai/docs/alerts-ui). In addition to using the Fiddler UI, users have the flexibility to set up alert rules using the Fiddler API client. In particular, the Fiddler client enables the following workflows:
...
Before creating a new alert rule, users choose the type of notification that will be leveraged by Fiddler when an alert is raised. Currently, Fiddler client supports email and PagerDuty services as notifications. To create a notification configuration we call the [build_notifications_config()](https://docs.fiddler.ai/reference/clientbuild_notifications_config) API. For example, the following code snippet creates a notification configuration using a comma separated list of email addresses.
...
Now let's sets up a Data Integrity alert rule which triggers an email notification when published events have 5% null values in any 1 hour bin for the _age_ column. Notice compare_to = 'raw_value'. The [add_alert_rule()](https://docs.fiddler.ai/reference/clientadd_alert_rule) API is used to create alert rules.
...
And the following API call sets up a Performance alert rule which triggers an email notification when the precision metric is 5% higher than that from 1 hr bin one day ago. Notice compare_to = 'time_period' and compare_period = '1 day'.
...
The [get_alert_rules()](https://docs.fiddler.ai/reference/clientget_alert_rules) API can be used to get a list of all alert rules with respect to the filtering parameters and it returns a paginated list of alert rules.
...
To delete an alert rule we need the corresponding unique **alert_rule_uuid** which is part of the output we get from  [get_alert_rules()](https://docs.fiddler.ai/reference/clientget_alert_rules). Then we can delete a rule by calling the [delete_alert_rule()](https://docs.fiddler.ai/reference/clientdelete_alert_rule)  API as shown below:
...
Finally, to get a paginated list of triggered alerts for a given alert rule in a given time range we can call the [get_triggered_alerts()](https://docs.fiddler.ai/reference/clientget_triggered_alerts) API as the following:
